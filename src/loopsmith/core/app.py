...
import loopsmith
from loopsmith.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from loopsmith.core.bus.envelope import EventPushEnvelope
from loopsmith.core.config import LoopSmithConfig, get_config
...
from loopsmith.core.permissions.storage import load_policy_file
from loopsmith.core.runner import AgentRunner
from loopsmith.core.runs import events_file, new_run_id
from loopsmith.core.session import SessionManager, SessionStore
from loopsmith.core.trace.record import TraceRecord
from loopsmith.core.trace.writer import TraceWriter
from loopsmith.core.transport.ipc_broadcaster import IpcEventBroadcaster
from loopsmith.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    def __init__(self) -> None:
        ...
        self._config: LoopSmithConfig | None = None
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=loopsmith.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in Path("~/.loopsmith/sessions").expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.loopsmith/policy.toml").expanduser()
        ...
        sessions_root = Path("~/.loopsmith/sessions").expanduser()
        ...
        logger.info("loopsmith-core %s listening addr=%s", loopsmith.__version__, addr)
    ...
    # Windows不支持 add_signal_handler，跳过异步信号注册
            pass
