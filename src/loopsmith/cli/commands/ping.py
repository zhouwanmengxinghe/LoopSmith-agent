...
import loopsmith
from loopsmith.core.bus.commands import PongResult
from loopsmith.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from loopsmith.core.config import LoopSmithConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: LoopSmithConfig) -> None:
    try:
...
async def _ping(config: LoopSmithConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{loopsmith.__version__}"},
    }
