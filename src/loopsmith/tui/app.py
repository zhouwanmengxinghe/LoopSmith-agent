...
from loopsmith.core.transport.socket_client import IpcError, SocketClient
...
class LoopSmithTuiApp(App[None]):
    """LoopSmith TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]LoopSmith[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update("[bold]LoopSmith[/bold]  [red]not connected — retrying...[/red]")
                await asyncio.sleep(2)
                continue

            self._client = client
            header.update(
                f"[bold]LoopSmith[/bold]  [dim]{self._host}:{self._port}[/dim]"
            )
            ...
            try:
                ...
                header.update(f"[bold]LoopSmith[/bold]  [red]subscribe error: {e}[/red]")
            finally:
            ...
            header.update("[bold]LoopSmith[/bold]  [dim]disconnected — retrying...[/dim]")
            await asyncio.sleep(2)
...
# TUI 入口：读取配置并启动 LoopSmithTuiApp
def run(config: LoopSmithConfig, replay_run_id: str | None = None) -> None:
    app = LoopSmithTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
